...
export function PostProvider({ children }) {
  ...
  const loadExplore = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      setError(
        "Falha ao carregar posts para explorar. Tente novamente mais tarde."
      );
      throw error;
    } finally {
      setLoading(false);
  ...
  const createPost = useCallback(
    async (postData) => {
      ...
      try {
        ...
        if (
          newPostData.author_id &&
          newPostData.author_id !== newPostData.user_id
        ) {
    ...
    async (postId) => {
      ...
      try {
        ...
        const filterArray = (array) =>
          array.filter((post) => post.id !== postId);
